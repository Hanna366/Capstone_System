...
export const NotificationCenter = ({ compact = false }: NotificationCenterProps) => {
  ...
  const modal = (
    <>
      ...
      <div className="fixed top-20 right-6 w-96 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl z-[100001]">
            ...
            {filteredEvents.length === 0 ? (
              ...
              <div className="space-y-2">
                {filteredEvents.map((event) => (
                  <div
                    key={event.id}
                    className="p-3 bg-slate-700/50 rounded-lg border border-slate-600/30 flex items-start gap-3"
                  >
  ...
  return (
    <>
      {/* Inline Notification Button - Aligned with other header buttons */}
      <div className="relative">
        <button
          onClick={toggleNotificationCenter}
          className="p-3 rounded-2xl backdrop-blur-md border shadow-xl transition-all duration-500 ease-out flex items-center justify-center w-12 h-12 group hover:scale-105"
        >
          <div className="relative">
            {unreadCount > 0 ? (
